...
class SettingTypeController extends Controller
{
    ...
    public function editAction()
    {
        ...
        if (IS_POST) {
            ...
            // 获取当前编辑的内容
            $setting_type_id = I('get.setting_type_id', '', 'trim');
            $this->assign('row', M('SettingType')->find($setting_type_id));
    ...
    public function multiAction()
    {
        ...
        switch ($operate) {
            case 'delete':
                // 使用in条件, 删除全部的品牌
                $cond = ['setting_type_id' => ['in', $selected]];
                M('SettingType')->where($cond)->delete();
                $this->redirect('list', [], 0);
                break;
            default:
                # code...
    ...
    public function ajaxAction()
    {
        ...
        switch ($operate) {
            ...
            case 'checkBrandUnique':
                ...
                // 获取模型后, 利用条件获取匹配的记录数
                $count = M('SettingType')->where($cond)->count();
