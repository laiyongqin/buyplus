...
class SettingGroupController extends Controller
{
    ...
    public function addAction()
    {
        ...
        if (IS_POST) {
            // 数据处理
            // $model = M('SettingGroup');
            $model = D('SettingGroup');
            $result = $model->create();
    ...
    public function editAction()
    {
        ...
        if (IS_POST) {
            ...
            // 获取当前编辑的内容
            $setting_group_id = I('get.setting_group_id', '', 'trim');
            $this->assign('row', M('SettingGroup')->find($setting_group_id));
    ...
    public function multiAction()
    {
        ...
        switch ($operate) {
            case 'delete':
                // 使用in条件, 删除全部的品牌
                $cond = ['setting_group_id' => ['in', $selected]];
                M('SettingGroup')->where($cond)->delete();
                $this->redirect('list', [], 0);
                break;
            default:
                # code...
    ...
    public function ajaxAction()
    {
        ...
        switch ($operate) {
            ...
            case 'checkBrandUnique':
                ...
                // 获取模型后, 利用条件获取匹配的记录数
                $count = M('SettingGroup')->where($cond)->count();
