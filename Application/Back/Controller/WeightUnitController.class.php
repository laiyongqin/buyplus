...
class WeightUnitController extends Controller
{
    ...
    public function editAction()
    {
        ...
        if (IS_POST) {
            ...
            // 获取当前编辑的内容
            $weight_unit_id = I('get.weight_unit_id', '', 'trim');
            $this->assign('row', M('WeightUnit')->find($weight_unit_id));
    ...
    public function multiAction()
    {
        ...
        switch ($operate) {
            case 'delete':
                // 使用in条件, 删除全部的品牌
                $cond = ['weight_unit_id' => ['in', $selected]];
                M('WeightUnit')->where($cond)->delete();
                $this->redirect('list', [], 0);
                break;
            default:
                # code...
    ...
    public function ajaxAction()
    {
        ...
        switch ($operate) {
            ...
            case 'checkBrandUnique':
                ...
                // 获取模型后, 利用条件获取匹配的记录数
                $count = M('WeightUnit')->where($cond)->count();
