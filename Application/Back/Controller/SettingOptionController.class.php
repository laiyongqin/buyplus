...
class SettingOptionController extends Controller
{
    ...
    public function addAction()
    {
        ...
        if (IS_POST) {
            // 数据处理
            // $model = M('SettingOption');
            $model = D('SettingOption');
            $result = $model->create();
    ...
    public function editAction()
    {
        ...
        if (IS_POST) {
            ...
            // 获取当前编辑的内容
            $setting_option_id = I('get.setting_option_id', '', 'trim');
            $this->assign('row', M('SettingOption')->find($setting_option_id));
    ...
    public function multiAction()
    {
        ...
        switch ($operate) {
            case 'delete':
                // 使用in条件, 删除全部的品牌
                $cond = ['setting_option_id' => ['in', $selected]];
                M('SettingOption')->where($cond)->delete();
                $this->redirect('list', [], 0);
                break;
            default:
                # code...
    ...
    public function ajaxAction()
    {
        ...
        switch ($operate) {
            ...
            case 'checkBrandUnique':
                ...
                // 获取模型后, 利用条件获取匹配的记录数
                $count = M('SettingOption')->where($cond)->count();
